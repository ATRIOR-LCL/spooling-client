            ...
            <div className="run-card">
                <div className="run-card-content">
                    ...
                    <div className="run-card-content-text">
                        <header className="run-card-content-text-header">{this.props.teamName}</header>
                        <footer className="run-card-content-text-footer">{this.props.taskTime}</footer>
                    </div>
                    <aside className="run-card-content-aside">
                        <p>Printer - {this.props.printerId}</p>
                        <button className="run-card-content-aside-close" onClick={this.props.toClose.bind(this, this.props.printerId)}>
...
class TaskItems extends React.Component<any, TaskItemsState> {
    ...
    public closeTask = (printerId: number): void => {
        this.setState((prevState) => ({
            TaskItemsArray: prevState.TaskItemsArray.filter(item => item.printerId !== printerId)
        }));
    }
    render(): React.ReactNode {
        return (
            <div className="run-task">
                ...
                <div className="run-task-items">
                    <div className="run-task-items-content">
                        {
                            this.state.TaskItemsArray.map((item, index) => {
                                return <TaskCard key={index} printerId={item.printerId} teamName={item.teamName} taskTime={item.taskTime} toClose={(n) => this.closeTask(n)} />
                            })
                        }
                    </div>
                </div>
    ...
    render(): React.ReactNode {
        return (
            <div className="run-task">
                ...
                <div className="run-task-items">
                    <div className="run-task-items-content"></div>
                </div>
            </div>
        )
    }
